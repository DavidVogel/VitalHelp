...
namespace vital {
  ...
  void ModulationConnectionProcessor::init() {
    /**
     * @brief Initializes parameters for bipolar, stereo, and bypass controls.
     */
  ...
  void ModulationConnectionProcessor::process(int num_samples) {
    /**
     * @brief Handles choosing between control-rate or audio-rate processing,
     * and prepares modulation inputs for further processing.
     */
  ...
  void ModulationConnectionProcessor::processAudioRate(int num_samples, const Output* source) {
    /**
     * @brief Determines which audio-rate processing method to call based on power and remapping usage.
     */
    if (bypass_->value()) {
  ...
  // The following methods implement different audio-rate processing strategies:
  // - processAudioRateLinear: no remapping, no morphing
  // - processAudioRateMorphed: morphing power applied, no remapping
  // - processAudioRateRemapped: remapping via LineGenerator, no morphing
  // - processAudioRateRemappedAndMorphed: both remapping and morphing power

  void ModulationConnectionProcessor::processAudioRateLinear(int num_samples, const Output* source) {
    // Basic linear scaling and bipolar offset.
  ...
  void ModulationConnectionProcessor::processAudioRateMorphed(int num_samples, const Output* source,
                                                              poly_float power) {
    // Applies a power curve to shape the modulation in addition to linear scaling and bipolar offset.
    poly_float* dest = output(kModulationOutput)->buffer;
    const poly_float* modulation_source = source->buffer;
    poly_float bipolar_offset = -bipolar_->value() * 0.5f;
  ...
  void ModulationConnectionProcessor::processAudioRateRemappedAndMorphed(int num_samples, const Output* source,
                                                                         poly_float power) {
    // Combines remapping and morphing power to nonlinearly shape the modulation.
    poly_float* dest = output(kModulationOutput)->buffer;
    const poly_float* modulation_source = source->buffer;
    poly_float bipolar_offset = -bipolar_->value() * 0.5f;
  ...
  void ModulationConnectionProcessor::processAudioRateRemapped(int num_samples, const Output* source) {
    // Remaps the modulation value via the LineGenerator before linear scaling and bipolar offset.
    poly_float* dest = output(kModulationOutput)->buffer;
    const poly_float* modulation_source = source->buffer;
  ...
  void ModulationConnectionProcessor::processControlRate(const Output* source) {
    /**
     * @brief Processes the modulation once per block at control-rate.
     *
