...
namespace vital {
  ...
  namespace {
    /**
     * @brief A scaling constant used in flange calculations (1/sqrt(2)).
     */
    constexpr float kFlangeScale = 0.70710678119f;

    /**
     * @brief Computes the low-frequency gain from a blend value in [0..2].
     *
     * @param blend A blend value, typically 0..2 for low->high transition.
     * @return Clamped low gain in [0..1].
     */
    ...
    /**
     * @brief Computes the high-frequency gain from a blend value in [0..2].
     *
     * @param blend A blend value, typically 0..2 for low->high transition.
     * @return Clamped high gain in [0..1].
     */
    ...
    /**
     * @brief The main tick function for the standard Comb feedback style.
     *
     * @param audio_in            Input sample.
     * @param memory              Pointer to the delay Memory buffer.
     * @param filter1             OnePoleFilter for the feedback’s low freq stage.
     * @param filter2             OnePoleFilter for secondary filtering.
     * @param period              Delay period in samples.
     * @param feedback            Feedback multiplier.
     * @param scale               Input scaling factor.
     * @param filter_coefficient  Coefficient for filter1.
     * @param filter2_coefficient Coefficient for filter2.
     * @param low_gain            Gain for low frequencies in the blend.
     * @param high_gain           Gain for high frequencies in the blend.
    ...
    force_inline poly_float tickComb(poly_float audio_in, Memory* memory,
      ...
      // Push the result into the memory buffer
    ...
    /**
     * @brief Tick function for a positive flanging style feedback.
     *
     * @param audio_in            Input sample.
     * @param memory              Delay memory buffer.
     * @param filter1             OnePoleFilter for feedback path (low freq).
     * @param filter2             Another OnePoleFilter for extended feedback or band spread.
     * @param period              Delay period in samples.
     * @param feedback            Feedback multiplier.
     * @param scale               Input scaling factor.
     * @param filter_coefficient  Coefficient for filter1.
     * @param filter2_coefficient Coefficient for filter2.
     * @param low_gain            Low-frequency blend gain.
     * @param high_gain           High-frequency blend gain.
     * @return The processed flanged output sample.
     */
    ...
    /**
     * @brief Tick function for a negative flanging style feedback.
     *
     * @param audio_in            Input sample.
     * @param memory              Delay memory buffer.
     * @param filter1             OnePoleFilter for feedback path (low freq).
    ...
    force_inline poly_float tickNegativeFlange(poly_float audio_in, Memory* memory,
                                               ...
                                               poly_float filter_coefficient, poly_float filter2_coefficient,
                                               poly_float low_gain, poly_float high_gain) {
      // Use half the period for the negative flange
  ...
  /**
   * @brief Constructs a CombFilter with default feedback style and sets up memory.
   *
   * @param size Initial size of the delay memory.
   */
  ...
  /**
   * @brief Copy constructor. Clones the memory buffer and resets filter states.
   *
   * @param other The CombFilter to copy.
   */
  ...
  /**
   * @brief Default destructor for CombFilter.
   */
  CombFilter::~CombFilter() { }

  /**
   * @brief Resets internal buffers and states for voices indicated by the reset_mask.
   *
   * @param reset_mask A poly_mask specifying which voices to reset.
   */
  ...
  /**
   * @brief Resets the filter for all voices.
   */
  ...
  /**
   * @brief Updates the filter parameters (feedback style, resonance, cutoff, etc.) based on FilterState.
   *
   * @param filter_state Struct with all relevant filter parameters.
   */
  void CombFilter::setupFilter(const FilterState& filter_state) {
    ...
    // Set feedback and scale based on style
    ...
    // Determine the filter style (band spread vs. low-high blend)
    FilterStyle filter_style = getFilterStyle(filter_state.style);
    if (filter_style == kBandSpread) {
      // Compute band offsets from center
  ...
  /**
   * @brief Processes audio for the entire block, calling the specific comb/flange tick function.
   *
  ...
  /**
   * @brief Templated processing routine choosing the per-sample tick function.
   *
   * @tparam tick A function pointer for the comb/flange tick (e.g., tickComb).
   * @param num_samples Number of samples to process.
   */
  ...
  void CombFilter::processFilter(int num_samples) {
    ...
    // Compute min frequency for bounding
    ...
    for (int i = 0; i < num_samples; ++i) {
      // Compute frequency from MIDI buffer
      ...
      // Execute the per-sample tick function
