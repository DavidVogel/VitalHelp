...
namespace vital {
    ...
    void Envelope::processControlRate(int num_samples) {
        /**
         * @brief Process the envelope at control rate.
         *
         * At control rate, the envelope updates its parameters once per control block.
         * It checks for triggers and state transitions, updating the envelope’s current value, position,
         * and phase state. Curves are applied based on the power parameters for attack, decay, and release.
         */
    ...
    poly_float Envelope::processSection(poly_float* audio_out, int from, int to,
        ...
        /**
         * @brief Process a segment of the envelope at audio rate.
         *
         * This function writes envelope values into the output buffer from `from` to `to`. It
         * applies a power curve to the normalized position and interpolates the envelope between
         * start and end values. After processing, it returns the new position.
         */
    ...
    void Envelope::processAudioRate(int num_samples) {
        /**
         * @brief Process the envelope at audio rate.
         *
         * This method updates the envelope sample-by-sample. It checks for triggers
         * and transitions at each audio block, computing the appropriate segment (delay,
         * attack, hold, decay, release, kill) and applying power curves and interpolation.
         */
