...
namespace vital {
  ...
  force_inline void SynthLfo::processTrigger() {
    /**
     * @brief Handles note trigger events, reset conditions, and updating masks and offsets.
     *
     * If a note-on occurs, it can reset the LFO offset depending on the sync type. If a note-off occurs,
     * it updates masks to reflect that the LFO may now be in a release state. Also handles delay and fade resets.
     */
    ...
    if (reset_mask.anyMask()) {
      ...
      if (sync_type == kSync) {
        // For sync mode, align offset with the external time reference.
        ...
        // For non-sync, simply reset offset to zero (or adjust by the triggered sample offset).
  ...
  void SynthLfo::processControlRate(int num_samples) {
    /**
     * @brief Processes the LFO at control rate.
     *
     * This happens typically once per block (not per sample). It updates delays, fades, smoothing,
     * and applies the selected sync mode logic to compute a single output value.
     */
    ...
    int sync_type = input(kSyncType)->at(0)[0];
    // Apply logic based on sync type
    ...
    if (input(kSmoothMode)->at(0)[0]) {
      // Smoothing the output value over time using exponential decay based on half-life.
      ...
      // If no smoothing, interpolate based on fade amplitude from the start phase.
  ...
  /*
  * The following audio-rate methods (processAudioRateEnvelope, processAudioRateSustainEnvelope, etc.)
  * follow a similar pattern: they integrate the phase and offset over each sample,
  * apply smoothing, fades, and delays, and write out a processed buffer of LFO values.
  *
  * They each handle a different sync mode, applying that mode's rules for
  * looping, holding, or envelope stopping, and produce a continuous buffer of LFO samples.
  */
  ...
  void SynthLfo::process(int num_samples) {
    /**
     * @brief Top-level processing method that decides between control-rate and audio-rate processing.
     *
     * If transitioning from control-rate to audio-rate, it initializes the audio-rate state from the control-rate state.
     * Then it handles triggers, and depending on the rate, calls the appropriate processing methods.
     */
  ...
  void SynthLfo::correctToTime(double seconds) {
    /**
     * @brief Updates the LFO's sync time reference, used for synchronization modes.
     */
