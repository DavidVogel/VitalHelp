...
/**
 * @brief Handles a crash in Vital by writing the current stack backtrace to a crash log file.
 *
 * @param data Unused pointer parameter, required by the system crash handler signature.
 */
...
/**
 * @brief Parses command-line arguments to find the value associated with a given flag.
 *
 * @param args The array of command-line arguments.
 * @param flag A short flag (e.g., "-m").
 * @param full_flag A long flag (e.g., "--midi").
 * @return The value associated with the given flag if found, or an empty string otherwise.
 */
...
/**
 * @brief Loads an audio file from an InputStream into an AudioSampleBuffer.
 *
 * The function attempts to read the audio data using available AudioFormatReaders. On success,
 * the destination buffer is resized and populated with the file's samples.
 *
 * @param destination The AudioSampleBuffer to load data into.
 * @param audio_stream A pointer to the input stream containing audio data.
 * @return The sample rate of the loaded file if successful, or 0 if loading failed.
 */
...
/**
 * @brief Determines the fading style to use when interpreting a wavetable from data strings.
 *
 * This function looks for special markers in the wavetable data and returns a corresponding
 * FileSource::FadeStyle to indicate how frames should be interpolated.
 *
 * @param data The wavetable data string.
 * @return The appropriate FileSource::FadeStyle derived from the data.
 */
 ...
 * @brief The main application class for Vital's standalone version.
 *
 * SynthApplication sets up the command-line interface, the main window (if not running headless),
 * and handles tasks such as exporting wavetables, rendering audio to files, and showing help/version
 * information.
 */
class SynthApplication : public JUCEApplication {
  public:
    /**
     * @class MainWindow
     * @brief The main window of the Vital standalone application.
     *
     * This window hosts the SynthEditor, which provides the GUI for interacting with the Vital
     * synthesizer engine. It supports file loading, preset saving/loading, and handles various
     * application commands (e.g., open, save).
     */
    class MainWindow : public DocumentWindow, public ApplicationCommandTarget, private AsyncUpdater {
      public:
        /**
         * @enum PresetCommand
         * @brief Enumeration of supported preset-related commands.
         */
        enum PresetCommand {
          kSave = 0x5001,    ///< Save current preset.
          kSaveAs,           ///< Save current preset as a new file.
          kOpen,             ///< Open a preset file.
          kToggleVideo,      ///< Toggle zoom for recording.
        };

        /**
         * @brief Constructs a MainWindow instance.
         *
         * Initializes the SynthEditor and GUI components. If @p visible is true, the window
         * will be shown with native title bars and sizing constraints.
         *
         * @param name The name of the window.
         * @param visible Whether the window is visible upon creation.
         */
        ...
        /**
         * @brief Called when the user attempts to close the main window.
         *
        ...
        /**
         * @brief Queues a file to be loaded once the application is ready.
         *
        ...
        /**
         * @brief Shuts down audio processing (e.g., before quitting).
         */
        void shutdownAudio() {
        ...
        /**
         * @brief Registers all commands (Save, Save As, Open) that this window can handle.
         *
         * @param commands The array to populate with available commands.
         */
        ...
        /**
         * @brief Provides metadata and default key bindings for commands.
         *
         * @param commandID The command to describe.
         * @param result The structure to fill with command info.
         */
         ...
         *
         * @param info Information about the command invocation.
        ...
        /**
         * @brief Called asynchronously to handle queued operations (like file loading).
         */
      ...
      private:
        /**
         * @brief Loads a file asynchronously once the UI is ready.
         */
        ...
        File file_to_load_;                                          ///< The file queued for loading.
        SynthEditor* editor_;                                        ///< The main synthesizer editor component.
        std::unique_ptr<ApplicationCommandManager> command_manager_; ///< Manages application commands and key mappings.
        BorderBoundsConstrainer constrainer_;                        ///< Constrains window resizing and aspect ratio.
    ...
    /**
     * @brief Indicates whether multiple instances of this application are allowed.
     * @return True if multiple instances are allowed; false otherwise.
     */
    bool moreThanOneInstanceAllowed() override { return true; }

    /**
     * @brief Initializes the application. Parses command-line options and may run headless tasks.
     *
     * Depending on flags, this may show version/help information, run table-to-wav conversion,
     * render images or audio, or launch the GUI.
     *
     * @param command_line The raw command-line string.
     */
    void initialise(const String& command_line) override {
      ...
      if (command.contains(" --version ") || command.contains(" -v ")) {
      ...
      #if JUCE_MODULE_AVAILABLE_juce_graphics
        // Render images from a wavetable
        static constexpr int kSampleRate = 88200;
        ...
        // Show the main window (unless headless is requested)
    ...
    /**
     * @brief Attempts to load a file passed in from the command line.
     *
     * @param command_line The file path or command-line argument to attempt to load.
    ...
    /**
     * @brief Called when the application is being shut down. Cleans up the main window.
     */
    ...
    /**
     * @brief Called when another instance of this application starts up.
     *
     * This attempts to load a file provided by the other instance's command line.
     *
     * @param command_line The command-line arguments from the new instance.
     */
  ...
  private:
    std::unique_ptr<MainWindow> main_window_; ///< The main window of the application.
};
