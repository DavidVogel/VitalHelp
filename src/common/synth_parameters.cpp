...
namespace vital {
  ...
  /**
   * @brief Comparison function for sorting parameters by version and name.
   *
   * This ensures that parameters are sorted first by the version in which they were added,
   * then alphabetically by name.
   *
   * @param a Pointer to the first ValueDetails.
   * @param b Pointer to the second ValueDetails.
   * @return True if a < b in sorting order.
   */
  ...
  // Various string and ID prefix constants used to name and organize parameter groups.
  ...
  // The constructor of ValueDetailsLookup populates the global parameter map and list.
  ValueDetailsLookup::ValueDetailsLookup() {
    ...
    // Add envelope groups (one for each envelope).
    ...
    // Add Random LFO parameter groups.
    ...
    // Add Oscillator parameter groups.
    ...
    // Add Modulation parameter groups.
    ...
    for (int modulation = kOldMaxModulations; modulation < kMaxModulationConnections; ++modulation) {
      // Newer modulation slots added in a newer version.
      addParameterGroup(mod_parameter_list, num_mod_parameters, modulation,
    ...
    // Sort the final parameter list by version and name.
  ...
  void ValueDetailsLookup::addParameterGroup(const ValueDetails* list, int num_parameters, std::string id,
    ...
    // Clone and prefix each parameter in the group with the provided identifiers and names.
    for (int i = 0; i < num_parameters; ++i) {
  ...
  // Static instance of the global lookup.
