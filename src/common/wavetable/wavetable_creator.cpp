...
namespace {
  /**
   * @brief Finds the first sample in a buffer that is non-zero.
   *
   * Useful for trimming silence from the start of an audio file before processing.
   *
   * @param audio_buffer The audio samples.
   * @param num_samples The number of samples in the buffer.
   * @return The index of the first non-zero sample, or 0 if none found.
   */
...
void WavetableCreator::postRender(float max_span) {
  // After rendering the entire table, apply normalization if requested.
  if (full_normalize_)
...
void WavetableCreator::renderToBuffer(float* buffer, int num_frames, int frame_size) {
  // Render multiple frames into a given buffer. Not fully implemented for different frame sizes.
...
void WavetableCreator::loadDefaultCreator() {
  // Create a default "Init" wavetable with a default group and render it.
...
void WavetableCreator::initPredefinedWaves() {
  // Load predefined wave shapes from a fixed set of waveforms.
  clear();
...
void WavetableCreator::initFromSplicedAudioFile(const float* audio_buffer, int num_samples, int sample_rate,
  ...
  // Load audio file as a spliced wavetable, dividing it into segments that map across the wavetable frames.
  clear();
...
void WavetableCreator::initFromVocodedAudioFile(const float* audio_buffer, int num_samples,
  ...
  // Vocoding approach attempts to detect a stable period and vocode a single cycle.
...
void WavetableCreator::initFromPitchedAudioFile(const float* audio_buffer, int num_samples, int sample_rate) {
  // Tries to detect a pitched period and map it across the wavetable frames.
  clear();
...
void WavetableCreator::initFromLineGenerator(LineGenerator* line_generator) {
  // Loads a line-based wave definition from a LineGenerator.
  clear();
...
bool WavetableCreator::isValidJson(json data) {
  // Checks if data matches either a LineGenerator or a full wavetable creator state.
...
json WavetableCreator::updateJson(json data) {
  // Update older versioned states to newer formats, ensuring backward compatibility.
  // Contains a series of conditional conversions based on version comparisons.
  ...
  if (LoadSave::compareVersionStrings(version, "0.3.3") < 0) {
    // Convert old component type ints to strings
  ...
  if (LoadSave::compareVersionStrings(version, "0.3.7") < 0) {
    // Convert PCM data to float buffers in components that use audio_file fields.
    json json_groups = data["groups"];
    json new_groups;
    for (json json_group : json_groups) {
      json json_components = json_group["components"];
  ...
  if (LoadSave::compareVersionStrings(version, "0.3.9") < 0 && LoadSave::compareVersionStrings(version, "0.3.7") >= 0) {
    // Convert PCM data to float data in wave_data fields for certain components.
    json json_groups = data["groups"];
    json new_groups;
    for (json json_group : json_groups) {
      json json_components = json_group["components"];
  ...
  if (LoadSave::compareVersionStrings(version, "0.7.7") < 0) {
    // Update old line source data to new line style including extra boundary points.
...
json WavetableCreator::stateToJson() {
  // Serialize the current wavetable creator state, including all groups and parameters.
  json json_groups;
  for (auto& group : groups_)
...
void WavetableCreator::jsonToState(json data) {
  // If data matches a single LineGenerator, treat this as a line-based initialization.
  if (LineGenerator::isValidJson(data)) {
  ...
  // Otherwise, treat data as full wavetable creator state.
  clear();
