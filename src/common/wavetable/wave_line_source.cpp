...
void WaveLineSource::WaveLineSourceKeyframe::interpolate(const WavetableKeyframe* from_keyframe,
  ...
  // Use pull_power_ to nonlinearize interpolation parameter 't'.
  ...
  // Interpolate each point and power value based on adjusted_t.
...
void WaveLineSource::WaveLineSourceKeyframe::render(vital::WaveFrame* wave_frame) {
  // Render the line-based waveform and map it from [0,1] to [-1,1].
...
void WaveLineSource::render(vital::WaveFrame* wave_frame, float position) {
  // Interpolate the compute_frame_ and render the final line-based waveform.
