...
namespace {
  ...
  /**
   * @brief Converts MSB and LSB values into a high-resolution float between 0.0f and 1.0f.
   *
   * @param msb The most significant byte of the MIDI control value.
   * @param lsb The least significant byte, or -1 if not present.
   * @return A float representing the combined high-resolution value.
   */
...
void MidiManager::midiInput(int midi_id, vital::mono_float value) {
  if (armed_value_) {
    // Assign the armed parameter to the received MIDI control.
