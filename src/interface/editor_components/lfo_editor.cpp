...
void LfoEditor::render(OpenGlWrapper& open_gl, bool animate) {
  // The rendering code sets up the LFO curve and the animated position indicator.
  // It applies smoothing (boost and decay) to changes in phase and amplitude.
  // The LFO curve is drawn along with optional stereo variations.
  ...
  if (animating) {
    // Drawing the animated LFO lines and positions.
    setFill(true);
    ...
    // Drawing the static LFO lines without animation.
    setBoostAmount(0.0f);
