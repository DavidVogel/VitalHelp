...
BarRenderer::BarRenderer(int num_points, bool vertical) :
    ...
    addRoundedCorners(); // Presumably a call from a base class or utility to add cornering.
    ...
    // Initialize bar geometry and indices.
...
void BarRenderer::drawBars(OpenGlWrapper& open_gl) {
    ...
    // Update buffer data if bars have changed.
    if (dirty_) {
        dirty_ = false;
        setBarSizes();
        open_gl.context.extensions.glBindBuffer(GL_ARRAY_BUFFER, bar_buffer_);
...
void BarRenderer::setBarSizes() {
    // Adjust the third coordinate to represent either width or height depending on orientation.
    if (vertical_) {
        for (int i = 0; i < total_points_; ++i) {
            int index = i * kFloatsPerBar;
