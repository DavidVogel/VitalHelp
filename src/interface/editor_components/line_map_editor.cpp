...
void LineMapEditor::render(OpenGlWrapper& open_gl, bool animate) {
  ...
  if (animate && animate_) {
    // Animated mode: decay boosts and update positions.
    ...
    setViewPort(open_gl);
    // Draw position indicators for both stereo voices if applicable.
    drawPosition(open_gl, position_color_stereo, phase[1]);
    drawPosition(open_gl, position_color, phase[0]);
  }
  else {
    // Non-animated mode: draw the lines statically.
    setBoostAmount(0.0f);
    setFillBoostAmount(0.0f);
    decayBoosts(0.0f);
