/// @file synth_slider.cpp
/// @brief Implements the SynthSlider class and related OpenGL slider components.
...
SynthSlider::SynthSlider(String name) : OpenGlSlider(name), show_popup_on_hover_(true), scroll_enabled_(true),
...
String SynthSlider::getSliderTextFromValue(double value) {
  ...
  if (popupSuffix_.isEmpty()) {
    return popup_prefix_ + formatValue(adjusted_value);
  } else {
    return popup_prefix_ + formatValue(adjusted_value) + popupSuffix_;
  }
}
