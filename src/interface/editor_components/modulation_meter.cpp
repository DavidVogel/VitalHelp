...
ModulationMeter::ModulationMeter(const vital::Output* mono_total, const vital::Output* poly_total,
  ...
  // If vertical bar or text/curve slider style, rotate coordinates in the multi-quad.
...
void ModulationMeter::resized() {
  ...
  if (parent) {
    // Check if the parameter is modulated by any source.
...
Rectangle<float> ModulationMeter::getMeterBounds() {
  ...
  // If not rotary and not text/curve, it's a linear slider: calculate a linear meter area.
  if (!destination_->isRotary() && !destination_->isTextOrCurve()) {
    ...
    if (destination_->isHorizontal()) {
      // Horizontal slider: meter goes vertically in the slider track.
      return Rectangle<float>(0.0f, outer_area + border, width, inner_area - outer_area - 2.0f * border);
    }

    // Vertical slider: meter goes horizontally in the slider track.
  ...
  // For text or curve style, use the entire area.
