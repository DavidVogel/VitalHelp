...
namespace {
  // Sorts selection arrays using a comparator.
  ...
  // Checks if a given root folder contains too many files.
  ...
  // Shows a warning if a folder has too many files.
...
void PopupDisplay::setContent(const std::string& text, Rectangle<int> bounds,
  ...
  String textToDisplay(text.c_str());

  // 1) Split lines:
  StringArray lines;
  lines.addLines(textToDisplay);

  // 2) Choose your base Font:
  static constexpr int kBaseHeight = 24;
  int height = kBaseHeight * size_ratio_;
  int mult = getPixelMultiple();
  Font font = Fonts::instance()->proportional_light().withPointHeight(height * 0.5f * mult);

  // 3) Measure widest line and total height:
  int maxWidth = 0;
  for (auto& line : lines) {
    maxWidth = jmax(maxWidth, font.getStringWidth(line));
  }

  // Add some margins/padding:
  int padding   = height / 4;
  int buffer    = padding * 2 + 2;
  int totalWidth = (maxWidth / getPixelMultiple()) + buffer;

  // Each line is about `height` tall, or you can do more precise measurement:
  int numLines    = lines.size();
  int lineHeight  = height; // or e.g. 1.2 * height if you want more spacing
  int totalHeight = lineHeight * numLines;

  // 4) Decide final bounds:
  // For instance, if bubble is above:
  int middle_x = bounds.getX() + bounds.getWidth() / 2;
  int middle_y = bounds.getY() + bounds.getHeight() / 2;

  if (placement == BubbleComponent::above) {
    setBounds(middle_x - totalWidth / 2,
              bounds.getY() - totalHeight,
    ...
    setBounds(bounds.getX() - totalWidth,
              middle_y - totalHeight / 2,
              totalWidth, totalHeight);
  } else if (placement == BubbleComponent::right) {
    setBounds(bounds.getRight(),
              middle_y - totalHeight / 2,
              totalWidth, totalHeight);
  }

  // 5) Actually set the text and font size in your PlainTextComponent
  text_.setText(text);              // keep the newlines
  text_.setJustification(Justification::centred);
