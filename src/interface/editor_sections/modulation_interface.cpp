...
namespace {
  // Strings representing keyboard modulation sources for top and bottom rows.
...
ModulationInterface::ModulationInterface(SynthGuiData* synth_data) : SynthSection("modulation") {
  ...
  // Create and configure envelope tab selector
  ...
  // Create and configure LFO tab selector
  ...
  // Create random modulation sections
  ...
  // Create and configure random tab selector
  ...
  // Keyboard modulation selectors (top and bottom rows)
  ...
  keyboard_modulations_top_->getButton(3)->overrideText("OCT NOTE"); // Special label
  ...
  keyboard_modulations_bottom_->getButton(0)->overrideText("PRESSURE"); // Special label
...
void ModulationInterface::paintBackground(Graphics& g) {
  ...
  // Draw backgrounds behind mod sections
...
void ModulationInterface::resized() {
  ...
  // Allocate vertical space based on minimum modulations shown
  ...
  // Adjust font sizes for selectors
...
void ModulationInterface::modulationSelected(ModulationTabSelector* selector, int index) {
  ...
  // Handle switching between different modulation tabs
