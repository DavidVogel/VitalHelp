...
namespace {
  /**
   * @brief The width (in GUI units) for the blend label icon.
   */
  constexpr int kBlendLabelWidth = 30;

  /**
   * @brief Returns the number of styles available for a given filter model.
   * @param int_model The integer representing the filter model.
   * @return The number of styles for that model.
   */
  ...
  /**
   * @brief Returns the style name as a string for a given model and style index.
   * @param int_model The integer representing the filter model.
   * @param style The style index for the given model.
  ...
  std::string getStyleName(int int_model, int style) {
    ...
    switch (model) {
      ...
      case vital::constants::kFormant:
        // Special handling for formant styles
...
FilterSection::FilterSection(String name, String suffix) :
  ...
  // Internal construction of the filter section common elements.
...
FilterSection::FilterSection(String suffix, const vital::output_map& mono_modulations) :
  ...
  // Construct a filter section with a filter response for mono modulations.
...
FilterSection::FilterSection(int index, const vital::output_map& mono_modulations,
  ...
  // Construct a filter section for a given index, using both mono and poly modulations.
  ...
  // Add oscillator and sample input toggle buttons
...
void FilterSection::setFilterResponseSliders() {
  // Link sliders to the FilterResponse so that the graph updates accordingly.
...
void FilterSection::paintBackground(Graphics& g) {
  // Paint the background and relevant labels, blend morph icons, etc.
...
void FilterSection::positionTopBottom() {
  // Lays out controls in a top-bottom orientation.
  int title_width = getTitleWidth();
...
void FilterSection::positionLeftRight() {
  // Lays out controls in a left-right orientation.
  int title_width = getTitleWidth();
  int knob_section_height = getKnobSectionHeight();
  int slider_width = getSliderWidth();
  int blend_label_width = size_ratio_ * kBlendLabelWidth;
...
void FilterSection::resized() {
  ...
  // Positioning formant and special sliders, setting label fonts, etc.
...
void FilterSection::buttonClicked(Button* clicked_button) {
  // Handle button clicks, including toggling filter serial mode, osc/sample inputs, etc.
  // Notify listeners as necessary.
...
void FilterSection::setAllValues(vital::control_map& controls) {
  // Set the filter model and style values from a control map and update the UI.
...
void FilterSection::prevClicked() {
  // Cycle to the previous filter model/style combination and notify of changes.
  current_style_--;
  if (current_style_ < 0) {
...
void FilterSection::nextClicked() {
  // Cycle to the next filter model/style combination and notify of changes.
  current_style_++;
...
void FilterSection::textMouseDown(const MouseEvent& e) {
  // Handle popup menu for selecting models and styles from text interaction.
  PopupItems options;
...
void FilterSection::setFilterSelected(int menu_id) {
  // Set the filter model/style from a given menu ID selection.
  int current_id = 1;
  for (int i = 0; i < vital::constants::kNumFilterModels; ++i) {
    int num_styles = getNumStyles(i);
...
void FilterSection::setOscillatorInput(int oscillator_index, bool input) {
  // Set the toggle state of the specified oscillator input.
...
void FilterSection::setSampleInput(bool input) {
  // Set the toggle state of the sample input.
  sample_input_->setToggleState(input, dontSendNotification);
}

void FilterSection::showModelKnobs() {
  // Show or hide knobs depending on the current filter model and style.
  vital::constants::FilterModel model = static_cast<vital::constants::FilterModel>(current_model_);
  filter_response_->setModel(model);
...
void FilterSection::setFilterText() {
  // Update the preset selector text to match the current model/style.
...
void FilterSection::setLabelText() {
  // Update the parameter labels depending on the current filter model.
...
void FilterSection::notifyFilterChange() {
  // Notify parent and FilterResponse of filter changes and update the synth values.
  filter_response_->setStyle(current_style_);
...
Path FilterSection::getLeftMorphPath() {
  // Returns a Path object representing the left morph icon for the current filter model/style.
...
Path FilterSection::getRightMorphPath() {
  // Returns a Path object representing the right morph icon for the current filter model/style.
  if (current_model_ == vital::constants::kPhase)
