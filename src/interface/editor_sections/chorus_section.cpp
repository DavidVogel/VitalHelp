/// @file chorus_section.cpp
/// @brief Implements the ChorusSection class, which provides a UI for configuring chorus effects.
...
/// @class ChorusViewer
/// @brief A visual display showing chorus delay lines based on the current settings.
///
/// The ChorusViewer draws vertical bars corresponding to each chorus voice's delay time. As the number
/// of voices or their parameters change, the visualization updates accordingly.
...
public:
    ...
    /// Initializes references to chorus delay outputs from the synth.
    ...
    /// Draws the delay bars based on the current chorus voice settings.
    ...
    void drawBars(OpenGlWrapper& open_gl, bool animate) {
        ...
        int num_voices = static_cast<int>(num_voices_->getValue() * vital::poly_float::kSize);
    ...
    /// Sets whether the chorus viewer is active.
    /// @param active True if active, false otherwise.
    void setActive(bool active) { active_ = active; }

    /// Sets the slider that controls the number of voices.
    /// @param num_voices The slider for the number of voices.
    void setNumVoicesSlider(SynthSlider* num_voices) { num_voices_ = num_voices; }

private:
    bool active_;                            ///< Whether the viewer is active.
    const vital::StatusOutput* delays_[4];    ///< Status outputs for chorus delay lines.
    SynthSlider* num_voices_;                 ///< Slider controlling the number of voices.
