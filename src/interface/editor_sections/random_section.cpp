...
/**
 * @class RandomViewer
 * @brief Visualizes the random modulation source as a waveform line renderer.
 *
 * RandomViewer is an OpenGlLineRenderer that shows a random source's changing values in real-time.
 * It supports stereo visualization and dynamic updates.
 */
class RandomViewer : public OpenGlLineRenderer {
  public:
  static constexpr int kResolution = 64;    ///< Number of points in the waveform visualization.
  static constexpr float kBoostAmount = 1.0f;  ///< Initial boost amount for the line rendering.
  static constexpr float kDecayMult = 0.9f;    ///< Decay multiplier for boost values over time.

    /**
     * @brief Constructs the RandomViewer with a given name.
     * @param name The name (identifying the random source).
     */
    ...
    /**
     * @brief Paints the background of the viewer, including a reference line.
    ...
    /**
     * @brief Renders the waveform lines. Updates positions based on the random source.
    ...
    /**
     * @brief Called when the parent hierarchy changes, used to find the parent synth and status output.
     */
    ...
    /**
     * @brief Called when the component is resized. Updates the stereo_line_ bounds.
     */
  ...
  private:
    SynthGuiInterface* parent_;               ///< Pointer to the parent GUI interface for retrieving the synth.
    OpenGlLineRenderer stereo_line_;          ///< A separate line renderer for the stereo channel.
    const vital::StatusOutput* random_value_; ///< Pointer to the random value output from the synth.
