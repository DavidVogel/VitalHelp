...
namespace {
  // Internal namespace for static strings, lookup tables, and helper functions.
  ...
  /**
   * @brief Gets suffix for distortion selection text.
   * @param type The distortion type.
   * @param index Oscillator index.
   * @return A suffix string indicating which oscillator modulation is used.
   */
  ...
  /**
   * @brief Checks if given spectral morph type index is a separator in the menu.
   * @param index The spectral morph type index.
   * @return True if separator, otherwise false.
   */
  ...
  /**
   * @brief Checks if given distortion type index is a separator in the menu.
   * @param index The distortion type index.
   * @return True if separator, otherwise false.
   */
  ...
  /**
   * @brief Builds a string for the distortion menu item.
   * @param type Distortion type.
   * @param index Oscillator index.
   * @return A localized display string for the distortion menu option.
   */
  ...
  /**
   * @brief Builds the distortion text string displayed on the UI.
   * @param type Distortion type.
   * @param index Oscillator index.
   * @return A display string for the current distortion type.
   */
  ...
  /**
   * @brief Gets the language index for a given language code.
   * @param language The language code.
   * @return The language index or the default index if not found.
   */
  ...
  /**
   * @brief Checks if the given distortion type is bipolar.
   * @param distortion_type The distortion type index.
  ...
  /**
   * @brief Checks if the given spectral morph type is bipolar.
   * @param morph_type The spectral morph type index.
   * @return True if bipolar, false otherwise.
   */
...
/**
 * @class UnisonViewer
 * @brief A component for visualizing unison voices distribution and detune.
 *
 * This class extends BarRenderer to display a representation of unison voices
 * and their detuning visually. It pulls modulation data from outputs and
 * adjusts the bar rendering accordingly.
 */
class UnisonViewer : public BarRenderer {
  public:
  /**
   * @brief Constructs a UnisonViewer.
   * @param index Oscillator index.
  ...
  /**
   * @brief Calculates total output values from given output pairs.
   * @param outputs Pair of mono and poly outputs.
   * @param default_value The default value if not animated.
   * @param animate Whether to use triggered output values.
   * @return A poly float representing the total output.
   */
  ...
  /**
   * @brief Sets the slider for voices.
   * @param slider Pointer to the SynthSlider controlling voices.
   */
    void setVoicesSlider(SynthSlider* slider) { voices_slider_ = slider; }

  /**
   * @brief Sets the slider for detune.
   * @param slider Pointer to the SynthSlider controlling detune.
   */
    void setDetuneSlider(SynthSlider* slider) { detune_slider_ = slider; }

  /**
   * @brief Sets the slider for detune power.
   * @param slider Pointer to the SynthSlider controlling detune power.
   */
    void setDetunePowerSlider(SynthSlider* slider) { detune_power_slider_ = slider; }

  /**
   * @brief Renders the unison visualization.
   * @param open_gl Reference to the OpenGlWrapper.
   * @param animate Whether to animate based on triggered values.
   */
  ...
  private:
  std::pair<vital::Output*, vital::Output*> voices_;       ///< Voices modulation outputs.
  std::pair<vital::Output*, vital::Output*> detune_;       ///< Detune modulation outputs.
  std::pair<vital::Output*, vital::Output*> detune_power_; ///< Detune power modulation outputs.

  SynthSlider* voices_slider_;       ///< Pointer to voices slider.
  SynthSlider* detune_slider_;       ///< Pointer to detune slider.
  SynthSlider* detune_power_slider_; ///< Pointer to detune power slider.
};

/**
 * @class InvisibleSlider
 * @brief A slider with no visible UI, used internally.
 */
class InvisibleSlider : public SynthSlider {
  public:
  /**
   * @brief Constructs an InvisibleSlider.
   * @param name The parameter name associated with this slider.
   */
    InvisibleSlider(String name) : SynthSlider(std::move(name)) { }

  /**
   * @brief Overridden to prevent drawing the slider graphics.
   * @param g The Graphics context.
   */
    void paint(Graphics& g) override { }

  /**
   * @brief Overridden to prevent drawing shadows.
   * @param g The Graphics context.
   */
    void drawShadow(Graphics& g) override { }
};

// Constructor implementation with UI component creation and configuration.
...
// Grab the suffix text from your map:
  auto& tooltips = getTooltips();
  auto it = tooltips.find("distortion_amount");
  if (it != tooltips.end()) {
    distortion_amount_->setPopupSuffix("\n" + it->second);  // prefix + value + suffix
  }
...
// Applies skin values to the wavetable display.
...
// Paints section background, including text editors and labels.
...
// Handles layout and positions all child components.
...
// Handles user interaction with buttons such as type selectors, navigation, etc.
...
// Updates local state from given control values and configures UI elements.
...
// Handles "Enter" pressed in TTWT text editor, attempts to convert text to wavetable.
...
// Handles focus loss in TTWT text editor.
...
// Used to hide error messages after a delay.
...
// Sets the active state for all relevant controls.
...
// Sets the displayed name in the preset selector.
void OscillatorSection::setName(String name) {
  preset_selector_->setText(name);
}

// Shows TTWT language settings.
...
// Loads a wavetable by sending text to a web service (TTWT).
...
// Updates the distortion selection and notifies listeners.
...
// Updates the spectral morph selection and notifies listeners.
...
// Loads the browser state from the parent component.
...
// Sets the currently selected index (no implementation details given).
...
// Sets the language for TTWT and updates preferences.
...
// Handles cancellation of language selection.
...
// Loads the previous wavetable file.
...
// Opens a popup browser on text click.
...
// Updates transpose quantization state.
...
// Resets distortion type if it's a modulation-based type.
...
// Loads wavetable from JSON data.
...
// Initiates text-to-wavetable input mode.
...
// Returns relative coordinates of the wavetable area.
...
// Configures spectral morph parameters and UI.
...
// Configures distortion parameters and UI.
...
// Configures destination parameters and notifies listeners.
...
// Shows or hides the distortion phase slider and updates layout.
...
// Notifies parent of spectral morph type change.
...
// Notifies parent and listeners of distortion type change.
...
// Notifies parent and listeners of destination change.
