...
namespace {
  // Lookup table for window shape names.
...
WaveWindowOverlay::WaveWindowOverlay() : WavetableComponentOverlay("WAVE WINDOW"), wave_window_modifier_(nullptr) {
  ...
  editor_->setVisible(false); // Only visible when a frame is selected.
  current_frame_ = nullptr;

  // Initialize the window shape selector.
  ...
  // Initialize the left and right position sliders.
  ...
  // Set up control background titles.
...
void WaveWindowOverlay::frameSelected(WavetableKeyframe* keyframe) {
  if (keyframe == nullptr) {
    // No keyframe selected: hide editor and reset current frame.
    editor_->setVisible(false);
    current_frame_ = nullptr;
  }
  else if (keyframe->owner() == wave_window_modifier_) {
    // Keyframe belongs to the WaveWindowModifier.
...
void WaveWindowOverlay::setEditBounds(Rectangle<int> bounds) {
  ...
  // Configure line and fill colors.
