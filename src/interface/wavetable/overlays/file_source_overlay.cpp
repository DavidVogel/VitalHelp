...
namespace {
  ...
  /**
   * @brief Converts window size text to a window size in samples.
   *
   * If the text represents a note name recognized by Tuning::noteToMidiKey,
   * it converts it to samples based on sample_rate and note frequency. Otherwise,
   * it parses the float value directly.
   *
   * @param text The input text from the text editor.
   * @param sample_rate The audio sample rate.
   * @return The window size in samples, clamped to a max value.
   */
  ...
  /**
   * @brief Converts position text to a start position in samples.
   * @param text The input text for start position.
   * @return The parsed start position in samples.
   */
