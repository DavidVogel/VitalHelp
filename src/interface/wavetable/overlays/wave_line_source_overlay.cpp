...
WaveLineSourceOverlay::WaveLineSourceOverlay() : WavetableComponentOverlay("LINE SOURCE"), line_source_(nullptr) {
  ...
  // Create a default line generator and associated line editor
  ...
  // Horizontal grid size slider and incrementers
  ...
  // Vertical grid size slider and incrementers
...
void WaveLineSourceOverlay::lineEditorScrolled(const MouseEvent& e, const MouseWheelDetails& wheel) {
  // Scroll modifies horizontal grid size
  if (wheel.deltaY > 0)
