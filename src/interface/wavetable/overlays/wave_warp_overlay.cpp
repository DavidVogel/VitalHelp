...
WaveWarpOverlay::WaveWarpOverlay() : WavetableComponentOverlay("WAVE WARPER"), warp_modifier_(nullptr) {
  ...
  // Initialize horizontal warp slider
  ...
  // Initialize vertical warp slider
  ...
  // Initialize horizontal asymmetry toggle
  ...
  // Initialize vertical asymmetry toggle
  ...
  // Set the controls layout titles
...
void WaveWarpOverlay::frameSelected(WavetableKeyframe* keyframe) {
  if (keyframe == nullptr) {
    ...
    // Keyframe belongs to the WarpModifier
...
void WaveWarpOverlay::setEditBounds(Rectangle<int> bounds) {
  ...
  // Set positions of the toggles and sliders
  ...
  // Set line separators for the background layout
...
void WaveWarpOverlay::sliderValueChanged(Slider* moved_slider) {
  ...
  // Update current frame's warp parameters
