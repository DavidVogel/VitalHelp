...
namespace {
  // Lookup table for filter styles display names.
...
FrequencyFilterOverlay::FrequencyFilterOverlay() :
  ...
  // Create and configure normalization toggle
  ...
  // Create and configure cutoff slider
  ...
  // Create and configure shape slider
  ...
  // Set up background lines and titles
...
void FrequencyFilterOverlay::frameSelected(WavetableKeyframe* keyframe) {
  if (keyframe == nullptr) {
    ...
    // Keyframe belongs to the same frequency modifier
...
bool FrequencyFilterOverlay::setFrequencyAmplitudeBounds(Rectangle<int> bounds) {
  // Not actively used, just return true
  return true;
}
