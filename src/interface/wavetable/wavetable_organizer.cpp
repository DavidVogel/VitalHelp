...
// Internal callback triggered by organizer context menu actions.
namespace {
...
WavetableOrganizer::WavetableOrganizer(WavetableCreator* wavetable_creator, int max_frames) :
  ...
  // Constructor sets up OpenGL quads and initial states.
...
// Draws background grid and sets colors for frame quads.
...
// Repositions frames and updates visuals after size changes.
...
void WavetableOrganizer::mouseDown(const MouseEvent& e) {
  // Handles selection, dragging initiation, and context menu starting positions.
...
void WavetableOrganizer::mouseDrag(const MouseEvent& e) {
  // Handles dragging frames or updating selection boxes.
...
void WavetableOrganizer::mouseUp(const MouseEvent& e) {
  // Finalizes drags, completes selection, and handles context menus.
  Component::mouseUp(e);
...
void WavetableOrganizer::mouseDoubleClick(const MouseEvent& e) {
  // Double-click can quickly add or remove a keyframe at a position.
  int row_index = getRowFromY(e.y);
  WavetableComponent* component = getComponentAtRow(row_index);
  if (component == nullptr || !component->hasKeyframes())
    return;
...
void WavetableOrganizer::playheadMoved(int position) {
  // Updates a visual marker to show where the playhead is currently located.
...
void WavetableOrganizer::componentRemoved(WavetableComponent* component) {
  // Ensures the organizer updates selection if a component is removed.
...
void WavetableOrganizer::componentAdded(WavetableComponent* component) {
  // Adds frames for the new component and selects its first frame if any.
  recreateVisibleFrames();
...
void WavetableOrganizer::componentsScrolled(int offset) {
  // Adjusts vertical offset after scrolling, repositioning frames accordingly.
